...
alltoto = re.findall(r'value="(\d\d\d\d)', html.decode('utf-8'))
for i in alltoto:
...
for i in alltoto:
    for i in i:
        ...
        # get locations
        if html.decode('utf-8').find('Group 1 has no winner') > 0:
            pass
        else:
            # cut html till group 1 winner only, ie. remove group 2 winners
            html = html[:html.decode('utf-8').find('Group 2 winning tickets')]
            soup = BeautifulSoup(html, 'html.parser')
            text = soup.select('li')
            try:
                for t in text:
                    ...
                    print(system)
                    cur.execute(
                        "INSERT OR IGNORE INTO place(draw_no,raw_data,location,address,quickpick,system) VALUES(?,?,?,?,?,?)",
                        (i, b, place, address, draw, system,))
            except:
                pass
        conn.commit()
